...
const FileUploadForm = () => {
  ...
  const handleFileChange = event => {
    setSelectedFile(event.target.files[0]) // Store only one file
  }
  ...
  const handleSubmit = async event => {
    ...
    formData.append('file', selectedFile) // Ensure this key matches backend @RequestParam("file")

    try {
      const res = await axios.post(`${API_URL}/extract`, formData)

      if (res.data) {
        const data = res.data?.replace('```json', '')
        localStorage.setItem('data', data?.replace('```', ''))
